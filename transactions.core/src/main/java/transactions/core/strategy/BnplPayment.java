...
public class BnplPayment implements IPaymentsMethod {
	...
	public String createCredit(Credit credit, GeneralInformation generalInformation, Context context,DynamoDBMapper mapper) {
		...
		BiFunction<Integer, String, List<String>> calcularFechas = (months, date) -> {
			...
			for (int i = 0; i < months; i++) {
				offsetDate = offsetDate.plusMonths(1).withHour(23).withMinute(59).withSecond(59);
		...
		double result = (credit.getRateApprobe() / 12) / 30;
		...
		loan.setDelinquencyInterestRate(generalInformation.getDelinquency_interest_rate()/100);
		loan.setOwner(basicInformation.getNames()+" "+basicInformation.getLastName());
		loan.setOriginationDate(dateFormatter);
		loan.setInterestRate(credit.getRateApprobe()/100);
		...
		credit.setDailyFeeCharge(formattedResult);
